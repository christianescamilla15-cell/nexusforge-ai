...
export const NEXUSFORGE_KB = {
  ...
  what_is_nexusforge: {
    ...
    question_patterns: ['what is nexusforge', 'que es nexusforge', 'about', 'overview', 'what does this do', 'para que sirve', 'explain nexusforge', 'explicame nexusforge', 'introduce', 'descripción', 'tell me about nexusforge', 'que hace esta plataforma'],
    ...
    answer_es: 'NexusForge AI es una plataforma de orquestación de agentes de grado empresarial. Coordina 22 agentes IA especializados a través de 6 topologías de ejecución (secuencial, paralelo, jerárquico, debate, consenso, adaptivo). La plataforma incluye un motor de ejecución DAG usando el algoritmo de Kahn para ordenamiento topológico, un sistema de memoria de 3 niveles (trabajo, episódica, semántica), auto-reparación con 5 estrategias de recuperación, un pipeline RAG con embeddings Voyage AI de 512 dimensiones almacenados en pgvector, enrutamiento LLM multi-proveedor de Groq a Claude con patrón circuit breaker, y monitoreo en tiempo real via WebSocket. El backend está construido con Python FastAPI y asyncio, el frontend con React 18 y Vite, respaldado por PostgreSQL y Redis. Está diseñado para que las organizaciones construyan flujos de trabajo IA complejos que sean observables, recuperables y eficientes en costos.'
  },
  ...
  how_it_works: {
    ...
    answer_es: 'Los datos entran a NexusForge a través de carga de documentos o llamada API. Los documentos se fragmentan en segmentos de 500 caracteres con 50 de solapamiento, luego se generan embeddings con Voyage AI en vectores de 512 dimensiones almacenados en pgvector. Los workflows definen pipelines DAG de pasos de agentes -- el ejecutor usa ordenamiento topológico (algoritmo de Kahn) para determinar el orden de ejecución. Los pasos sin dependencias mutuas se ejecutan en paralelo via asyncio.gather. Cada paso invoca un agente a través del router LLM multi-proveedor, que intenta Groq primero (40x más barato) y recurre a Claude si es necesario. Los resultados se guardan en Redis después de cada paso. Los eventos en tiempo real se transmiten via WebSocket. Si algún paso falla, el motor de auto-reparación detecta el tipo de error, selecciona una estrategia de recuperación y lo intenta automáticamente.'
  },
  ...
  use_cases: {
    ...
    answer_es: 'NexusForge soporta una amplia gama de casos de uso empresariales. **Procesamiento de documentos** -- clasificar, extraer entidades, resumir y validar documentos en un solo workflow. **Verificación de cumplimiento** -- ejecutar documentos a través del agente de cumplimiento. **Automatización de investigación** -- el agente investigador recopila información mientras el resumidor condensa hallazgos. **Clasificacion de datos** -- el agente clasificador categoriza documentos en legal, financiero, técnico, médico o general. **Generación de contenido** -- encadenar agentes planificador, investigador y reportero. **Traducción multi-idioma** -- el agente traductor maneja localización. **Aseguramiento de calidad** -- los agentes critico y validador puntuan y filtran la calidad.'
  },
  ...
  getting_started: {
    ...
    answer_en: 'Start by exploring the **Dashboard** to see platform stats like total agents, workflows, and executions. Then go to **Workflows** to see pre-built DAG pipelines -- try the "Análisis de Documentos" template which chains classifier, extractor, summarizer, and validator agents. Go to **Documents** to upload a file and watch the RAG pipeline chunk, embed, and index it in real-time. Use the **Agents** page to browse all 22 agents and see their configurations. Try **Swarms** to see how agents collaborate in different topologies like debate or consensus. Finally, check the **Analytics** page for execution metrics, cost tracking, and performance dashboards. Each section has interactive demos that work without API keys.',
    answer_es: 'Empieza explorando el **Dashboard** para ver estadísticas de la plataforma. Luego ve a **Workflows** para ver pipelines DAG pre-construidos -- prueba la plantilla "Análisis de Documentos" que encadena agentes clasificador, extractor, resumidor y validador. Ve a **Documentos** para subir un archivo y ver el pipeline RAG procesarlo en tiempo real. Usa la página de **Agentes** para explorar los 22 agentes. Prueba **Swarms** para ver como los agentes colaboran en topologías como debate o consenso. Finalmente, revisa **Analytics** para métricas de ejecución, seguimiento de costos y dashboards de rendimiento.'
  },
  ...
  agents_overview: {
    ...
    answer_es: 'NexusForge tiene **22 agentes especializados**, cada uno heredando de BaseAgent. Cada agente usa el router LLM multi-proveedor (Groq primario, Claude respaldo) y tiene modo demo de respaldo.\n\n**Procesamiento de Datos:** Clasificador, Extractor, Normalizador, Validador\n**Análisis:** Analizador, Resumidor, Sentimiento, Investigador\n**Inteligencia:** Planificador, Router, Crítico, Conocimiento\n**Operaciones:** Monitor, Reparacion, Programador, Webhook\n**Contenido:** Reportero, Traductor, OCR, Scraper\n**Calidad:** Enriquecedor, Cumplimiento\n\nCada agente recibe un dict de entrada estructurado, lo procesa a través de su prompt especializado y retorna una salida estandarizada con metadatos.'
  },

  classifier_agent: {
    category: 'agents',
    question_patterns: ['classifier', 'clasificador', 'classify', 'clasificar', 'document classification', 'clasificación de documentos', 'categorize', 'categorizar'],
    ...
    answer_es: 'El **Agente Clasificador** categoriza documentos en tipos predefinidos: legal, financiero, técnico, médico y general. Analiza los primeros 500 caracteres usando el LLM para determinar la categoría más probable. La salida incluye la categoría predicha, una puntuación de confianza de 0 a 1, y razonamiento detallado. Soporta listas de categorías personalizadas via el parámetro `categories`. En modo demo usa heurísticas basadas en palabras clave. La latencia típica es menor a 2 segundos con Groq. El clasificador suele ser el primer paso en workflows de procesamiento de documentos.'
  },
  ...
  extractor_agent: {
    ...
    answer_es: 'El **Agente Extractor** realiza reconocimiento de entidades nombradas, extrayendo personas, organizaciones, fechas, montos monetarios y ubicaciones del texto. Retorna una lista estructurada donde cada entidad incluye el tipo (PERSONA, ORG, FECHA, DINERO, UBICACIÓN), el valor extraído y el contexto circundante. Puede procesar documentos de hasta 10,000 caracteres dividiendolos en ventanas con solapamiento. Deduplica entidades que aparecen en múltiples ventanas. En modo demo usa patrones regex. El extractor típicamente se encadena después del clasificador.'
  },
  ...
  summarizer_agent: {
    ...
    answer_es: 'El **Agente Resumidor** genera resúmenes de longitud configurable con tres modos: corto (1-2 oraciones), medio (1 párrafo) y largo (múltiples párrafos con encabezados). La salida incluye el texto del resumen, un array de puntos clave y conteo de palabras. Maneja documentos de hasta 15,000 caracteres fragmentando, resumiendo cada fragmento y produciendo un resumen consolidado final. El parámetro `style` permite tono formal, casual o técnico. En modo demo extrae la primera y última oracion de cada párrafo.'
  },

  analyzer_agent: {
    category: 'agents',
    question_patterns: ['analyzer', 'analizador', 'analyze', 'analizar', 'analysis', 'análisis', 'sentiment analysis', 'topic detection', 'complexity'],
    ...
    answer_es: 'El **Agente Analizador** realiza análisis de texto multidimensional incluyendo detección de sentimiento (positivo, negativo, neutro con confianza), identificación de temas (top 5 temas con puntuaciónes), evalúacion de complejidad (nivel Flesch-Kincaid) y estadísticas de texto (conteo de palabras, oraciones, longitud promedio, riqueza de vocabulario). También puede detectar idioma y nivel de formalidad. En modo demo usa heurísticas de frecuencia de palabras.'
  },
  ...
  validator_agent: {
    ...
    answer_es: 'El **Agente Validador** actúa como puerta de calidad, puntuando la salida de agentes de 0 a 100. Verifica tres dimensiones: completitud, consistencia y precisión. Retorna un booleano `is_valid` (true si puntuación >= 70), la puntuación numérica, un array de problemas encontrados y un array de recomendaciones. El umbral es configurable. En workflows, el validador típicamente es el último paso -- si falla, el motor de auto-reparación puede activar un ciclo de reparación.'
  },

  repair_agent: {
    category: 'agents',
    question_patterns: ['repair agent', 'agente reparación', 'reparar', 'fix agent', 'arreglar', 'diagnose', 'diagnosticar', 'self-repair'],
    ...
    answer_es: 'El **Agente de Reparacion** es el núcleo del sistema de auto-reparación. Cuando un paso falla, recibe el mensaje de error, la configuración del paso fallido y la entrada original. Analiza la falla y retorna un diagnóstico estructurado con: descripción de causa raiz, tipo de corrección (reintentar, reconfigurar, omitir o escalar), cambios de configuración sugeridos, y un booleano can_auto_fix. Si can_auto_fix es true, el ejecutor aplica los cambios automáticamente y re-ejecuta el paso.'
  },

  planner_agent: {
    category: 'agents',
    question_patterns: ['planner', 'planificador', 'plan', 'planificar', 'decompose', 'descomponer', 'task planning', 'planificación de tareas', 'break down'],
    ...
    answer_es: 'El **Agente Planificador** descompone tareas complejas en una lista ordenada de subtareas con relaciones de dependencia. Dado un objetivo de alto nivel, retorna un array de plan de ejecución donde cada entrada tiene: nombre del paso, agente asignado (de los 22 disponibles), dependencias, prioridad (1-5) y complejidad estimada. El planificador entiende las capacidades de cada agente. Es el cerebro detrás de la topología jerárquica.'
  },

  critic_agent: {
    category: 'agents',
    question_patterns: ['critic', 'critico', 'critique', 'críticar', 'evalúate', 'evalúar', 'review', 'revisar', 'feedback'],
    answer_en: 'The **Critic Agent** evalúates output quality from other agents on a 0-100 scale. It provides a detailed critique with: overall score, strengths array (what was done well), improvements array (specific actionable suggestions), a pass/fail boolean (threshold at 75 by default), and a one-paragraph written review. The critic is central to the debate swarm topology -- an agent produces output, the critic evalúates it, and if the score is below threshold, the original agent tries again incorporating the critique. This loop continues until quality meets the standard or max iterations are reached.',
    answer_es: 'El **Agente Crítico** evalúa la calidad de salida de otros agentes en una escala de 0-100. Proporciona una crítica detallada con: puntuación general, array de fortalezas, array de mejoras (sugerencias específicas), booleano aprobado/reprobado (umbral en 75 por defecto), y una reseña escrita. El critico es central en la topología de debate -- un agente produce salida, el critico la evalúa, y si la puntuación es baja, el agente original intenta de nuevo incorporando la crítica.'
  },
  ...
  normalizer_agent: {
    ...
    answer_es: 'El **Agente Normalizador** limpia y estandariza datos de texto. Maneja unificacion de formatos de fecha (convirtiendo a ISO 8601), normalizacion de monedas, capitalizacion de nombres, limpieza de espacios, correcciónes de codificacion (normalizacion UTF-8) y eliminacion de lineas duplicadas. Retorna el texto limpio mas un registro de todas las transformaciónes aplicadas. Es esencial en pipelines donde la entrada viene de múltiples fuentes.'
  },
  ...
  enricher_agent: {
    ...
    answer_es: 'El **Agente Enriquecedor** aumenta datos extraídos con contexto adicional de la base de conocimiento y búsquedas externas. Dada una serie de entidades, consulta la memoria semántica y el pipeline RAG para encontrar información relacionada. Por ejemplo, si se extrae "Anthropic", podría agregar "Empresa de seguridad IA, fundada 2021, creadora de Claude." Retorna las entidades originales mas campos de enriquecimiento.'
  },

  researcher_agent: {
    category: 'agents',
    question_patterns: ['researcher', 'investigador', 'research', 'investigar', 'investigate', 'gather information', 'recopilar información'],
    ...
    answer_es: 'El **Agente Investigador** recopila y sintetiza información sobre un tema dado. Consulta el pipeline RAG para fragmentos de documentos relevantes, los combina con su conocimiento LLM, y produce un informe de investigación estructurado con: resumen ejecutivo, hallazgos clave, evidencia de soporte con referencias, brechas de conocimiento identificadas y próximos pasos recomendados. Se puede configurar con parámetro `depth` (superficial, medio, profundo).'
  },
  ...
  translator_agent: {
    ...
    answer_es: 'El **Agente Traductor** maneja la localización de contenido entre idiomas. Soporta traduccion entre ingles, espanol, frances, aleman, portugues y mas. Mas alla de la traduccion literal, preserva formato (markdown, vinetas, encabezados), adapta referencias culturales y mantiene consistencia en terminologia tecnica. La salida incluye texto traducido, idioma fuente detectado, idioma destino y puntuación de confianza.'
  },

  compliance_agent: {
    category: 'agents',
    question_patterns: ['compliance', 'cumplimiento', 'regulatory', 'regulatorio', 'legal check', 'verificación legal', 'audit', 'auditoria'],
    answer_en: 'The **Compliance Agent** checks documents against regulatory and policy requirements. It scans for PII exposure (names, SSNs, emails, phone numbers), validates data handling practices against GDPR/CCPA guidelines, checks for required legal disclaimers, and flags potentially problematic language. The output includes a compliance_score (0-100), a list of violations with severity (crítical, warning, info), specific text locations of issues, and remediation suggestions. The agent can be configured with custom rule sets for industry-specific compliance.',
    answer_es: 'El **Agente de Cumplimiento** verifica documentos contra requisitos regulatorios y de políticas. Escanea exposicion de PII (nombres, SSN, emails, telefonos), valida practicas de manejo de datos contra guias GDPR/CCPA, verifica disclaimers legales requeridos y senala lenguaje potencialmente problematico. La salida incluye compliance_score (0-100), lista de violaciones con severidad, ubicaciones en el texto y sugerencias de remediacion.'
  },

  monitor_agent: {
    category: 'agents',
    question_patterns: ['monitor', 'monitoring', 'monitoreo', 'observe', 'observar', 'health check', 'system health', 'salud del sistema'],
    answer_en: 'The **Monitor Agent** tracks system health and execution metrics in real-time. It collects agent latency, token usage, error rates, memory utilization, and queue depths. It can detect anomalies like sudden latency spikes or elevated error rates and trigger alerts. The monitor produces periodic health reports with: overall system status (healthy/degraded/crítical), per-agent performance stats, resource utilization percentages, and trend indicators. It feeds data to the Prometheus metrics endpoint which powers the 5 Grafana dashboards.',
    answer_es: 'El **Agente Monitor** rastrea la salud del sistema y métricas de ejecución en tiempo real. Recopila latencia de agentes, uso de tokens, tasas de error, utilización de memoria y profundidad de colas. Detecta anomalías como picos de latencia o tasas de error elevadas. Produce informes periódicos con: estado general del sistema, estadísticas por agente, porcentajes de utilización y indicadores de tendencia.'
  },

  router_agent: {
    category: 'agents',
    question_patterns: ['router agent', 'agente router', 'routing', 'enrutamiento', 'route tasks', 'enrutar tareas', 'task routing'],
    answer_en: 'The **Router Agent** intelligently dispatches tasks to the most suitable agent based on content analysis. Given an input document or task description, it evalúates the content type, complexity, and required capabilities, then selects the optimal agent or sequence of agents. It returns the recommended agent name, confidence in the routing decisión, reasoning, and an alternative agent if the primary choice is unavailable. The router is the brain behind the adaptive swarm topology -- it dynamically selects which topology pattern to use based on the task characteristics.',
    answer_es: 'El **Agente Router** despacha inteligentemente tareas al agente más adecuado basándose en análisis de contenido. Evalua tipo de contenido, complejidad y capacidades requeridas, luego selecciona el agente óptimo. Retorna el nombre del agente recomendado, confianza en la decisión, razonamiento y un agente alternativo. Es el cerebro detrás de la topología adaptiva.'
  },
  ...
  knowledge_agent: {
    ...
    answer_es: 'El **Agente de Conocimiento** gestiona la capa de memoria semántica y sirve como interfaz a la base de conocimiento. Puede almacenar nuevo conocimiento (embeber texto en pgvector), recuperar conocimiento relevante (búsqueda semántica), actúalizar entradas y identificar brechas de conocimiento. Retorna fragmentos relevantes con puntuaciónes de similitud, referencias a documentos fuente y timestamps.'
  },

  scraper_agent: {
    category: 'agents',
    question_patterns: ['scraper', 'web scraper', 'scraping', 'web scraping', 'crawl', 'rastrear', 'extract web', 'extraer web'],
    answer_en: 'The **Scraper Agent** extracts structured data from web pages and HTML content. It can parse HTML to extract text content, tables, links, metadata (title, description, OpenGraph tags), and specific CSS-selector-targeted elements. The output includes cleaned text, extracted tables as arrays, link inventory, and metadata dict. It handles common challenges like JavaScript-rendered content indicators, págination hints, and encoding detection. The scraper feeds data into the RAG pipeline for indexing and is often the first step in research automation workflows.',
    answer_es: 'El **Agente Scraper** extrae datos estructurados de páginas web y contenido HTML. Puede parsear HTML para extraer texto, tablas, enlaces, metadatos (titulo, descripción, OpenGraph) y elementos especificos por selector CSS. La salida incluye texto limpio, tablas como arrays, inventario de enlaces y dict de metadatos. Alimenta datos al pipeline RAG para indexacion.'
  },
  ...
  ocr_agent: {
    ...
    answer_es: 'El **Agente OCR** extrae texto de imágenes y documentos escaneados. Procesa formatos comunes (PNG, JPG, TIFF) y PDFs escaneados. La salida incluye texto extraído, puntuación de confianza por bloque, idioma detectado e información de diseño de página. Maneja diseños multi-columna, tablas en imágenes y texto rotado.'
  },
  ...
  sentiment_agent: {
    ...
    answer_es: 'El **Agente de Sentimiento** realiza análisis granular de sentimiento más allá de la clasificación simple positivo/negativo. Detecta: sentimiento general con confianza, tono emocional (alegria, enojo, tristeza, sorpresa, miedo, confianza), sentimiento por aspecto, nivel de intensidad (leve, moderado, fuerte) y detección de sarcasmo/ironia.'
  },
  ...
  scheduler_agent: {
    ...
    answer_es: 'El **Agente Programador** gestiona ejecuciones programadas y recurrentes de workflows. Soporta programación estilo cron, triggers por intervalo, triggers por evento y ejecución diferida única. Mantiene una cola de prioridad de ejecuciones pendientes y maneja zonas horarias. Retorna recibos de ejecución con: hora_programada, workflow_id, tipo_trigger y próxima_ejecución.'
  },

  webhook_agent: {
    category: 'agents',
    question_patterns: ['webhook', 'webhooks', 'external trigger', 'trigger externo', 'api trigger', 'integration', 'integración'],
    ...
    answer_es: 'El **Agente Webhook** maneja integraciónes de webhooks entrantes y salientes. Para entrantes, valida payloads contra esquemas JSON configurables, extrae datos relevantes y activa workflows. Para salientes, envia resultados a sistemas externos con políticas de reintento. Soporta verificación HMAC, headers personalizados y plantillas de transformación. Permite integrar NexusForge con Slack, GitHub, Jira y sistemas empresariales.'
  },
  ...
  workflows_intro: {
    ...
    answer_es: 'Los Workflows en NexusForge son **grafos aciclicos dirigidos (DAGs)** que encadenan múltiples agentes en pipelines de procesamiento. Cada nodo es un "paso" que invoca un agente especifico con entradas configuradas. Las aristas representan dependencias. Como el grafo es aciclico, el sistema determina un orden de ejecución valido usando ordenamiento topológico. Los pasos sin dependencias mutuas se ejecutan en paralelo automáticamente. Los workflows soportan plantillas, versionado, y pueden guardarse, compartirse, clonarse y programarse.'
  },

  dag_engine: {
    category: 'workflows',
    question_patterns: ['dag engine', 'motor dag', 'kahn', 'topological sort', 'ordenamiento topológico', 'execution engine', 'motor de ejecución', 'how does dag work', 'como funciona el dag'],
    ...
    answer_es: 'El motor de ejecución DAG usa el **algoritmo de Kahn** para ordenamiento topológico. El algoritmo: (1) calcula el grado de entrada de cada paso, (2) agrega pasos con grado 0 a la cola de listos, (3) ejecuta pasos listos en paralelo via asyncio.gather, (4) al completar un paso, decrementa el grado de sus dependientes, (5) repite hasta completar todo. Incluye **detección de ciclos**, **validacion de dependencias faltantes** y **detección de pasos duplicados**. Cada paso completado se guarda en Redis.'
  },
  ...
  create_workflow: {
    ...
    answer_en: 'To create a workflow, go to the **Workflows** page and click "New Workflow." You can start from scratch or use a template. Available templates include: **Análisis de Documentos** (classifier -> extractor -> summarizer -> validator), **Research Pipeline** (researcher -> analyzer -> summarizer -> critic), **Content Generation** (planner -> researcher -> reporter -> translator -> validator), and **Data Processing** (OCR -> normalizer -> classifier -> extractor -> enricher). Each step has configurable parameters: input source (previous step output, manual, or document), agent-specific settings, retry policy, and timeout. Use the visual editor to drag and connect steps, or switch to JSON mode for complex configurations.',
    answer_es: 'Para crear un workflow, ve a la página de **Workflows** y haz clic en "Nuevo Workflow." Puedes empezar desde cero o usar una plantilla. Plantillas disponibles: **Análisis de Documentos** (clasificador -> extractor -> resumidor -> validador), **Pipeline de Investigacion** (investigador -> analizador -> resumidor -> critico), **Generación de Contenido** (planificador -> investigador -> reportero -> traductor -> validador) y **Procesamiento de Datos** (OCR -> normalizador -> clasificador -> extractor -> enriquecedor).'
  },

  workflow_execution: {
    category: 'workflows',
    question_patterns: ['run workflow', 'ejecutar workflow', 'workflow execution', 'ejecución de workflow', 'what happens when run', 'que pasa al ejecutar', 'execute', 'ejecutar'],
    answer_en: 'When you click **Run** on a workflow: (1) The DAG is validated -- cycle detection, dependency check, agent availability. (2) An execution record is created with a unique run_id. (3) The topological sort identifies the first group of parallelizable steps. (4) Each step is dispatched to its assigned agent via the LLM router. (5) Real-time status updates broadcast via WebSocket -- you see each step transition through queued -> running -> completed/failed. (6) Completed step outputs are checkpointed to Redis and passed as inputs to dependent steps. (7) If a step fails, the self-healing engine evalúates and applies recovery. (8) When all steps finish, the final output is assembled with results, total duration, tokens used, and cost.',
    answer_es: 'Al hacer clic en **Ejecutar**: (1) Se valida el DAG. (2) Se crea un registro de ejecución con run_id unico. (3) El ordenamiento topológico identifica el primer grupo de pasos paralelizables. (4) Cada paso se despacha a su agente via el router LLM. (5) Actualizaciones en tiempo real via WebSocket. (6) Las salidas se guardan en Redis y se pasan como entradas a pasos dependientes. (7) Si un paso falla, el motor de auto-reparación evalúa y aplica recuperación. (8) Al finalizar, la salida final se ensambla con resultados, duración, tokens y costo.'
  },

  checkpointing: {
    category: 'workflows',
    question_patterns: ['checkpoint', 'checkpointing', 'punto de control', 'crash recovery', 'recuperación de fallos', 'resume', 'reanudar', 'dead letter', 'cola de muertos'],
    ...
    answer_es: 'NexusForge implementa **checkpointing a nivel de paso** para resiliencia. Despues de que cada paso completa, su estado se serializa y guarda en Redis con TTL de 24 horas. Si el proceso falla, el ejecutor puede reanudar desde el último checkpoint. Los pasos que fallan permanentemente van a la **cola de mensajes muertos** (DLQ) en Redis Stream, que captura configuración, entrada, detalles del error y todos los intentos de recuperación.'
  },
  ...
  swarms_intro: {
    category: 'swarms',
    question_patterns: ['swarm', 'swarms', 'enjambre', 'enjambres', 'topologies', 'topologías', 'explain swarms', 'explicar enjambres', 'explain topologies', 'explicar topologías', 'explica las topologías', 'what are swarms', 'que son los swarms', 'collaboration patterns', 'patrones de colaboración'],
    answer_en: 'Swarms are **multi-agent collaboration patterns** that define how agents work together. While workflows define WHAT steps to run, swarm topologies define HOW agents collaborate. NexusForge supports **6 topologies**:\n\n1. **Sequential** -- A->B->C chain, output of one feeds the next\n2. **Parallel** -- Fan-out to N agents simultaneously, fan-in to merge results\n3. **Hierarchical** -- Planner decomposes task, workers execute in parallel, synthesizer combines\n4. **Debate** -- Agent produces output, Critic evalúates, loop until quality threshold met\n5. **Consensus** -- N agents solve independently, Judge evalúates all and picks best\n6. **Adaptive** -- Router analyzes task complexity and dynamically selects the best topology\n\nEach topology supports checkpointing, configurable retry policies, timeout limits, and real-time progress streaming via WebSocket.',
    answer_es: 'Los Swarms son **patrones de colaboración multi-agente** que definen como los agentes trabajan juntos. NexusForge soporta **6 topologías**:\n\n1. **Secuencial** -- Cadena A->B->C, la salida de uno alimenta al siguiente\n2. **Paralelo** -- Fan-out a N agentes simultaneamente, fan-in para fusionar resultados\n3. **Jerarquico** -- Planificador descompone tarea, trabajadores ejecutan en paralelo, sintetizador combina\n4. **Debate** -- Agente produce salida, Crítico evalúa, ciclo hasta alcanzar umbral de calidad\n5. **Consenso** -- N agentes resuelven independientemente, Juez evalúa y elige el mejor\n6. **Adaptivo** -- Router analiza complejidad y selecciona dinamicamente la mejor topología\n\nCada topología soporta checkpointing, políticas de reintento configurables, limites de timeout y streaming de progreso en tiempo real via WebSocket.'
  },

  topology_sequential: {
    category: 'swarms',
    question_patterns: ['sequential topology', 'topología secuencial', 'chain', 'cadena', 'sequential swarm', 'enjambre secuencial', 'a then b then c'],
    ...
    answer_es: 'La **topología Secuencial** es el patrón más simple: los agentes ejecutan uno tras otro en orden definido (A->B->C). La salida de cada agente se convierte en la entrada del siguiente. **Cuando usar:** Pipelines de procesamiento directos donde cada paso depende del anterior. **Ejemplo:** Un documento legal pasa por Clasificador -> Extractor -> Resumidor -> Validador.'
  },

  topology_parallel: {
    category: 'swarms',
    question_patterns: ['parallel topology', 'topología paralela', 'fan out', 'fan in', 'concurrent', 'concurrente', 'parallel swarm', 'enjambre paralelo', 'simultaneous'],
    ...
    answer_es: 'La **topología Paralela** usa un patrón fan-out/fan-in: una entrada se envia a múltiples agentes simultaneamente, luego los resultados se fusionan. **Cuando usar:** Cuando necesitas múltiples análisis independientes del mismo documento. **Internamente:** asyncio.gather ejecuta todos los agentes concurrentemente. Estrategia de fusion configurable: "concat", "merge_dict" o "custom". **Ejemplo:** Un documento se analiza simultaneamente por Analizador, Extractor, Sentimiento y Resumidor.'
  },

  topology_hierarchical: {
    category: 'swarms',
    question_patterns: ['hierarchical topology', 'topología jerárquica', 'hierarchical swarm', 'enjambre jerárquico', 'planner workers', 'planificador trabajadores', 'decompose task'],
    ...
    answer_es: 'La **topología Jerarquica** usa un enfoque de tres fases: Planificar -> Ejecutar -> Sintetizar. **Fase 1:** El Planificador descompone la tarea en subtareas. **Fase 2:** Los trabajadores ejecutan subtareas, las independientes corren en paralelo. **Fase 3:** Un sintetizador combina todas las salidas en un resultado final coherente. **Cuando usar:** Tareas complejas que se benefician de dividir y conquistar.'
  },

  topology_debate: {
    category: 'swarms',
    question_patterns: ['debate topology', 'topología debate', 'debate swarm', 'enjambre debate', 'iterative improvement', 'mejora iterativa', 'agent critic loop'],
    answer_en: 'The **Debate topology** creates an iterative improvement loop between a producer agent and the Critic agent. **How it works:** (1) The producer agent generates initial output. (2) The Critic agent evalúates it, scoring 0-100 with specific feedback. (3) If the score is below the quality threshold (default 75), the producer receives the critique and generates an improved version. (4) This loop repeats until the score passes the threshold or max iterations (default 3) are reached. **When to use:** Tasks where quality is crítical and iterative refinement adds value, like content generation or complex analysis. **Example:** Summarizer produces a report -> Critic scores it 62 ("missing financial details") -> Summarizer revises -> Critic scores 81 ("comprehensive, clear") -> Output accepted. Typical improvement is 15-25 points between first and final iteration.',
    answer_es: 'La **topología de Debate** crea un ciclo de mejora iterativa entre un agente productor y el agente Crítico. **Como funciona:** (1) El productor genera salida inicial. (2) El Crítico la evalúa, puntuando 0-100. (3) Si la puntuación esta bajo el umbral (default 75), el productor recibe la crítica y genera una version mejorada. (4) El ciclo se repite hasta pasar el umbral o alcanzar maximo de iteraciones (default 3). **Ejemplo:** Resumidor produce informe -> Crítico puntua 62 -> Resumidor revisa -> Crítico puntua 81 -> Salida aceptada. Mejora típica de 15-25 puntos entre primera y última iteracion.'
  },

  topology_consensus: {
    category: 'swarms',
    question_patterns: ['consensus topology', 'topología consenso', 'consensus swarm', 'enjambre consenso', 'voting', 'votación', 'judge', 'juez', 'multiple opinions'],
    answer_en: 'The **Consensus topology** gathers multiple independent opinions and selects the best. **How it works:** (1) N agents (typically 3-5) receive the same task independently -- they don\'t see each other\'s work. (2) Each agent produces its output. (3) A Judge agent (usually the Critic) evalúates all outputs, scoring each one. (4) The Judge selects the winner based on highest score and writes a justification. **When to use:** High-stakes decisións where you want to reduce bias and increase reliability. **Example:** 3 Analyzer agents independently assess a financial report -> Judge evalúates all three -> Selects the most thorough analysis -> Confidence increases from ~80% (single agent) to ~95% (consensus). The tradeoff is 3x the token cost for significantly higher reliability.',
    answer_es: 'La **topología de Consenso** reune múltiples opiniones independientes y selecciona la mejor. **Como funciona:** (1) N agentes reciben la misma tarea independientemente. (2) Cada agente produce su salida. (3) Un agente Juez evalúa todas las salidas. (4) El Juez selecciona la ganadora con justificacion. **Cuando usar:** Decisiones de alto riesgo donde quieres reducir sesgo. **Ejemplo:** 3 Analizadores evalúan un informe -> Juez selecciona el mas completo -> Confianza aumenta de ~80% a ~95%. El tradeoff es 3x el costo de tokens.'
  },

  topology_adaptive: {
    category: 'swarms',
    question_patterns: ['adaptive topology', 'topología adaptiva', 'adaptive swarm', 'enjambre adaptivo', 'dynamic topology', 'topología dinamica', 'auto select', 'auto seleccionar'],
    answer_en: 'The **Adaptive topology** is the most intelligent pattern -- it dynamically selects the best topology based on task analysis. **How it works:** (1) The Router agent analyzes the incoming task for complexity, type, time constraints, and quality requirements. (2) Based on this analysis, it selects one of the other 5 topologies. (3) The selected topology executes normally. **Selection logic:** Simple tasks -> Sequential. Independent multi-analysis -> Parallel. Complex decomposable tasks -> Hierarchical. Quality-crítical content -> Debate. High-stakes decisións -> Consensus. **When to use:** When you have diverse task types and don\'t want to manually select topologies. The adaptive topology adds ~500ms overhead for the routing decisión but ensures optimal execution patterns. It learns from past executions via episodic memory to improve routing over time.',
    answer_es: 'La **topología Adaptiva** es el patrón mas inteligente -- selecciona dinamicamente la mejor topología basándose en análisis de la tarea. **Como funciona:** (1) El agente Router analiza la tarea. (2) Selecciona una de las otras 5 topologías. (3) La topología seleccionada ejecuta normalmente. **Logica de seleccion:** Tareas simples -> Secuencial. Multi-análisis independiente -> Paralelo. Tareas complejas -> Jerarquico. Calidad crítica -> Debate. Decisiones de alto riesgo -> Consenso. **Cuando usar:** Cuando tienes diversos tipos de tareas. Agrega ~500ms de overhead pero asegura patrones óptimos.'
  },
  ...
  memory_overview: {
    ...
    answer_es: 'NexusForge usa un **sistema de memoria de 3 niveles** disenado para dar a los agentes contexto a corto plazo y aprendizaje a largo plazo. **Memoria de Trabajo** mantiene el contexto actúal en proceso. **Memoria Episodica** almacena resúmenes de tareas en Redis con TTL de 30 días. **Memoria Semantica** embebe conocimiento como vectores de 512 dimensiones en pgvector permanentemente. Los tres niveles trabajan juntos: la de trabajo provee contexto inmediato, la episódica permite aprender de éxitos y fallos recientes, y la semántica da acceso al conocimiento acumulado.'
  },

  working_memory: {
    category: 'memory',
    question_patterns: ['working memory', 'memoria de trabajo', 'short term', 'corto plazo', 'current context', 'contexto actúal', 'in process memory'],
    ...
    answer_es: '**Memoria de Trabajo** es el nivel mas rapido, almacenado en proceso (dict de Python). Mantiene: descripción de tarea actúal, últimos 20 mensajes, resultados intermedios de pasos anteriores y configuración del agente. Tiene alcance de una sola ejecución -- al completar el workflow, se descarta. La latencia es sub-milisegundo. Capacidad maxima de aproximadamente 50KB por ejecución.'
  },

  episodic_memory: {
    category: 'memory',
    question_patterns: ['episodic memory', 'memoria episódica', 'task history', 'historial de tareas', 'redis memory', 'learning from past', 'aprender del pasado'],
    ...
    answer_es: '**Memoria Episodica** almacena registros estructurados de ejecuciones pasadas en Redis con TTL de 30 días. Cada episodio contiene: tipo de tarea, agente usado, resumen de entrada/salida, estado éxito/fallo, detalles de error, duración y costo de tokens. Los agentes consultan la memoria episódica antes de ejecutar para encontrar tareas similares. Latencia típica de 2-5ms.'
  },

  semantic_memory: {
    category: 'memory',
    question_patterns: ['semantic memory', 'memoria semántica', 'vector memory', 'memoria vectorial', 'pgvector memory', 'long term', 'largo plazo', 'permanent memory'],
    ...
    answer_es: '**Memoria Semantica** es la capa de conocimiento permanente, usando Voyage AI para embeber texto en vectores de 512 dimensiones almacenados en pgvector de PostgreSQL. Almacena: fragmentos de documentos del pipeline RAG, insights de agentes, resúmenes de conocimiento y entradas de la base de conocimiento. Las consultas usan similitud coseno. No tiene TTL -- el conocimiento persiste indefinidamente. Permite compartir conocimiento entre agentes. Usa indices IVFFlat para búsqueda eficiente.'
  },
  ...
  healing_overview: {
    category: 'healing',
    question_patterns: ['self-healing', 'auto-reparación', 'self healing', 'healing system', 'sistema de reparación', 'error recovery', 'recuperación de errores', 'automatic recovery', 'recuperación automatica', 'heal'],
    ...
    answer_es: 'El **motor de auto-reparación** de NexusForge detecta, diagnostica y recupera automáticamente de fallas sin intervención humana. El sistema tiene tres componentes: (1) **FailureDetector** clasifica errores en 6 tipos, (2) **StrategySelector** elige la estrategia óptima de 5 opciones, (3) **RecoveryExecutor** aplica la estrategia y verifica éxito. Reduce el tiempo de recuperación de horas a segundos y alimenta patrones a la memoria episódica.'
  },

  error_detection: {
    category: 'healing',
    question_patterns: ['error types', 'tipos de error', 'failure detection', 'detección de fallos', 'error classification', 'clasificación de errores', 'what errors', 'que errores'],
  ...
  healing_strategies: {
    ...
    answer_es: 'El motor usa **5 estrategias de recuperación**:\n\n1. **Reintento** -- Re-ejecutar con parámetros modificados. Backoff exponencial (base 2s, max 30s). Para: errores de red, timeouts. Max 3 reintentos.\n2. **Omitir** -- Generar salida por defecto y continuar. Cuando: el paso es opcional.\n3. **Reparar** -- El Agente de Reparacion diagnostica y sugiere cambios. Para: errores de calidad de datos, esquemas incorrectos.\n4. **Escalar** -- Enviar a cola de revision humana. Para: errores de sistema, fallos repetidos.\n5. **Respaldo** -- Usar resultados en cache de una ejecución éxitosa previa (similitud > 0.85).'
  },

  healing_example: {
    category: 'healing',
    question_patterns: ['healing example', 'ejemplo de reparación', 'healing walkthrough', 'show me healing', 'muestrame reparación', 'healing in action', 'concrete example'],
  ...
  llm_router: {
    ...
    answer_es: 'El **Router LLM** gestiona acceso multi-proveedor con failover inteligente. **Primario: Groq** con Llama 3.3 70B -- extremadamente rapido (200+ tokens/seg) y 40x más barato que Claude. **Respaldo: Claude Sonnet** -- mayor calidad para razonamiento complejo. Implementa un **patrón circuit breaker**: cuando los errores de Groq exceden 50% en 60 segundos, el circuito se "abre" y todo va a Claude. Despues de 30 segundos, una prueba verifica Groq. También rastrea costo por solicitud.'
  },

  rag_pipeline: {
    category: 'technical',
    question_patterns: ['rag', 'rag pipeline', 'pipeline rag', 'retrieval augmented', 'generacion aumentada', 'vector search', 'búsqueda vectorial', 'document search', 'búsqueda documentos', 'embedding', 'embeddings'],
  ...
  cost_tracking: {
    ...
    answer_es: '**Seguimiento de costos** integrado en cada ejecución:\n\n**Groq (Llama 3.3 70B):** $0.59/M tokens entrada, $0.79/M salida. Llamada promedio: ~$0.0008.\n**Claude Sonnet:** $3.00/M entrada, $15.00/M salida. Llamada promedio: ~$0.0084.\n\nEl router intenta **Groq primero** (40x más barato). Un workflow tipico de 5 pasos cuesta ~$0.004 en Groq vs ~$0.042 en Claude. El dashboard muestra desglose por agente, workflow, periodo y proveedor.'
  },
  ...
  comparison: {
    ...
    answer_es: '**NexusForge vs alternativas:**\n\n**vs LangChain:** LangChain es una libreria; NexusForge es una plataforma completa con orquestación, memoria, auto-reparación, monitoreo y UI.\n\n**vs CrewAI:** NexusForge ofrece 6 topologías (CrewAI tiene 2), memoria de 3 niveles, auto-reparación con 5 estrategias y funciones enterprise.\n\n**vs n8n/Zapier:** NexusForge es AI-nativo -- los agentes razonan, se adaptan y aprenden.\n\n**vs AutoGen:** NexusForge agrega infraestructura de produccion: ejecución DAG, checkpointing, monitoreo, costos, plugins y despliegue Kubernetes.'
  },
  ...
  project_stats: {
    category: 'about',
    question_patterns: ['stats', 'estadísticas', 'numbers', 'numeros', 'how big', 'que tan grande', 'project size', 'tamano del proyecto', 'metrics', 'métricas', 'scope'],
