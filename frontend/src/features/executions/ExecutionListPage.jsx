...
export default function ExecutionListPage({ onSelectExecution }) {
  ...
  if (loading) {
    return (
      <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#E5E7EB', marginBottom: 8 }}>Ejecuciones</h1>
        <p style={{ fontSize: 14, color: '#9CA3AF', marginBottom: 24 }}>Cargando...</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#E5E7EB', marginBottom: 4 }}>Ejecuciones</h1>
          <p style={{ fontSize: 14, color: '#9CA3AF' }}>
            Monitorea todas las ejecuciones de workflows en tiempo real.
            <span style={{ marginLeft: 8, color: '#6366F1' }}>{executions.length} total</span>
          </p>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              {['Estado', 'Workflow', 'Inicio', 'Duracion', 'Costo', 'Pasos'].map((h) => (
                <th key={h} style={{
          ...
          <tbody>
            ...
            {filtered.map((exec) => (
              <tr
                ...
                aria-label={`Ver ejecucion ${exec.run_id}`}
              >
                ...
                <td style={{ padding: '12px 16px', color: '#9CA3AF', fontSize: 13 }}>{formatDate(exec.started_at)}</td>
