...
export default function RunsDashboard() {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px', fontFamily: '-apple-system, sans-serif' }}>
      ...
      {health && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
          {[
            ...
            <div key={i} style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 12, padding: 20 }}>
            ...
            {runs.map((run, i) => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 12, color: '#6B7280' }}>{run.total_latency_ms ? `${run.total_latency_ms}ms` : '—'}</span>
                  <span style={{
                    fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 100,
                    background: statusColor(run.status) + '15', color: statusColor(run.status),
                  }}>{run.status}</span>
                </div>
              </div>
            ))}
                ...
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    background: e.event.includes('fail') ? '#DC2626'
                      : e.event.includes('fallback') ? '#F59E0B'
                      : e.event.includes('retry') ? '#F59E0B'
                      : e.event.includes('completed') ? '#059669'
                      : '#2563EB',
                  }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{e.event}</div>
                    {e.agent && <div style={{ fontSize: 12, color: '#6B7280' }}>agent: {e.agent}</div>}
          ...
          <div style={{ border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden' }}>
            ...
            {health.agents.map((a, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', padding: '14px 20px', borderTop: '1px solid #F3F4F6', fontSize: 14, color: '#374151' }}>
                ...
                <span style={{ color: a.success_rate >= 0.9 ? '#059669' : a.success_rate >= 0.7 ? '#F59E0B' : '#DC2626' }}>
